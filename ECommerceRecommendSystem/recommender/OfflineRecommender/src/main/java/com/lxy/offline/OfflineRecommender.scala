...
object OfflineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // TODO 核定计算过程(对于该程序是最重要的部分)
    //1、定义隐语义模型
    val trainData = ratingRDD.map(x => Rating(x._1, x._2, x._3))
    // 定义模型训练的参数，rank隐特征个数，iterations 迭代次数，正则化系数
    val (rank, iterations, lambda) = (5, 10, 0.01)
    val model = ALS.train(trainData, rank, iterations, lambda)

    //2、获取预测评分矩阵，得到用户的推荐列表
    // 用userRDD 和productRDD 做一个笛卡尔积，得到空的userProductsRDD
    ...
    val userRecs = preRating.filter(_.rating > 0)
      ...
      .map {
        case (userId, recs) =>
          UserRecs(userId, recs.toList.sortWith(_._2 > _._2).take(USER_MAX_RECOMMONDATION).map(x => Recommendation(x._1, x._2)))
      }
      .toDF()
    ...
    //3、利用商品的特征向量(DoubleMatrix)，计算商品的相似度列表
